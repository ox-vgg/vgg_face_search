...
# $2 -> Full path to video (if input type is "video") or full path to text file containing list of images to ingest (if input type is "images")
# $3 -> Full path to base folder holding the images referenced by the search service
...
export PATH=$PATH:/home/ernesto/VISOR/software/ffmpeg-3.2/bin
BASEDIR=$(dirname "$0")
cd "$BASEDIR"
source ../bin/activate
if [ "$1" = "video" ]; then
